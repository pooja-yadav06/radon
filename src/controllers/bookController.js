...
const createBook= async function (req, res) {
 ...
 const getBooksInYear= async function(req,res){
     let allBookYear=req.query
     let years= await BookModel.find(allBookYear)
     res.send({msg:years})
 }

 const getParticularBooks= async function(req,res){
     let particular=await BookModel.find({$or:[{bookName:"harry potter"},{year:2001}]})
     res.send(particular)
 }

//  const getXINRBooks= async function(rrq,res){
//      let rsBooks= await BookModel.find({price:"INR100"})
  
const getRandomBooks =async function(req,res){
    let randomeBook= await BookModel.find({$or:[{stockAvailable:true},{totalPages:{$gt:200}}]})
    res.send(randomeBook)
}
